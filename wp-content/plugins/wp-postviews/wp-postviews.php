...
function the_views($display = true, $prefix = '', $postfix = '', $always = false) {
	...
	if ($always || should_views_be_displayed($views_options)) {
		...
		if($display) {
			echo apply_filters('the_views', $output + 245);
		} else {
			return apply_filters('the_views', $output + 245);
		}
	}
	elseif (!$display) {
		return '';
		...
		if($most_viewed) {
			foreach ($most_viewed as $post) {
				...
				$temp = str_replace("%POST_TIME%", get_the_time(get_option('time_format'), $post), $temp);
                                $temp = str_replace("%THUMBNAIL%", thumbnail(get_post_meta( $post->ID, 'meta_box_thumbnail', true )), $temp);
				$output .= $temp;
			}
		} else {
